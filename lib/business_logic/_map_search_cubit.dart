...
class _MapSearchCubit extends Cubit<_MapSearchState> {
  ...
  Future<GeocodeResponse> getGeocodeData(Map<String, dynamic> queryParameters) async {
    try {
      Response response = await Dio().get(ApiProvider.geocodeFinder, queryParameters: queryParameters);
  ...
  Future<void> getAutoSearchData(Map<String, dynamic> queryParameters) async {
    ...
    try {
      Response response = await Dio().get(ApiProvider.autocompleteSearch, queryParameters: queryParameters);
      if (response.data?['status'] == 'REQUEST_DENIED') {
        throw (response.data['error_message']);
      } else {
        ...
        print(response.realUri);
        print(response.data);
        emit(AutoCompleteSearchResponse(AutoCompleteModel.fromJson(response.data)));
      }
    } on DioError catch (e) {
